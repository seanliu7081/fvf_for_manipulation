...
class PolarImplicitPolicyCE(BasePolicy):
    """
    ...
    def compute_loss(self, batch):
        ...
        # Compute circular energy function for the given obs and action magnitudes
        r = targets[:, :, 0, 0]
        phi = self.normalizer["action"].unnormalize(targets)[:, :, 0, 1]
        polar_act = torch.concatenate([r.view(B, N, 1), phi.view(B, N, 1)], axis=2)
        ...
        # Compute Cross-Entropy loss with one-hot labels
        # Apply log_softmax to energy scores
        log_probs = F.log_softmax(energy, dim=1)
        
        # Cross-entropy with one-hot labels: -sum(one_hot * log_probs) / batch_size
        # ce_loss = -(one_hot * log_probs).sum(dim=1).mean()
        
        # Alternative: You can also use KL divergence which is equivalent for one-hot targets
        ce_loss = F.kl_div(log_probs, one_hot, reduction='batchmean')
        
        loss = ce_loss

        return loss, ce_loss, torch.tensor(0.0), torch.tensor(0.0)
