...
#             if isinstance(info, dict) and info.get("is_success", 0) > 0:
...
#         return str(video_path), episode_rewards, episode_success
...
#                             if info[i].get('is_success', 0) > 0:
#                                 all_successes[start + i] = True
...
#             mean_success = np.mean(success_list[prefix])  # NEW: Success rate
#             log_data[prefix + "mean_score"] = mean_success  # Use success rate as mean_score
#             log_data[prefix + "mean_reward"] = mean_reward  # Keep reward separate
#             print(f"{prefix}mean_score (success rate) = {mean_success:.3f}")
...
""" drone_runner.py - FIXED version with success rate tracking """
...
class DroneRunner(BaseRunner):
    ...
    def _run_single_env_with_video(self, global_idx, policy, device):
        ...
        episode_rewards = []
        episode_success = False  # NEW: Track if episode succeeded
        past_action = None
        policy.reset()
        ...
        while step_count < self.max_steps:
            ...
            if done or truncated:
                # Check success ONLY at episode end using info['is_success']
                # This avoids false positives from intermediate steps
                if isinstance(info, dict):
                    is_success_val = info.get('is_success', 0)
                    # Debug: print what we received
                    if isinstance(is_success_val, np.ndarray):
                        print(f"[DEBUG] is_success array: shape={is_success_val.shape}, values={is_success_val[:5]}... last={is_success_val[-1] if len(is_success_val) > 0 else 'empty'}")
                        # Check LAST value, not any()
                        if len(is_success_val) > 0 and is_success_val[-1] > 0:
                            episode_success = True
                    else:
                        print(f"[DEBUG] is_success scalar: {is_success_val}")
                        if is_success_val > 0:
                            episode_success = True
                break
        ...
        print(f"[VIDEO] Completed: {len(episode_rewards)} steps, total_reward={sum(episode_rewards):.3f}, success={episode_success}")
        
        return str(video_path), episode_rewards, episode_success  # NEW: Return success

    def _run_single_env_without_video(self, global_idx, policy, device):
        """Run a single environment WITHOUT video recording"""
        seed = self.env_seeds[global_idx]
        prefix = self.env_prefixs[global_idx]
        
        # Create base environment (no GUI, no video)
        if self.env_name == "go_to_target":
            base_env = GoToTargetEnv(gui=False, record=False)
        elif self.env_name == "fly_through_gate":
            base_env = FlyThroughGateEnv(gui=False, record=False)
        ...
        # Wrap with MultiStepWrapper only (no video wrapper)
        wrapped_env = MultiStepWrapper(
            base_env,
            n_obs_steps=self.num_obs_steps + self.num_latency_steps,
            n_action_steps=self.num_action_steps,
        ...
        while step_count < self.max_steps:
            ...
            if done or truncated:
                # Check success ONLY at episode end
                if isinstance(info, dict):
                    is_success_val = info.get('is_success', 0)
                    if isinstance(is_success_val, np.ndarray):
                        # Check LAST value, not any()
        ...
        return None, episode_rewards, episode_success
    ...
    def run(
        ...
        all_video_paths = [None] * num_inits
        all_rewards = [None] * num_inits
        all_successes = [False] * num_inits  # NEW: Track success for each env
        energy_fn_plots = [list() for _ in range(num_inits)]
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Handle environments with video recording separately
            # But still evaluate ALL environments, not just video ones
            if any(need_recording):
                for idx in range(this_num_active_envs):
                    global_idx = start + idx
                    ...
                    if need_recording[idx]:
                        ...
                        # Run WITHOUT video recording (but still evaluate!)
                        _, rewards, success = self._run_single_env_without_video(
                            global_idx, policy, device
                        )
                        all_rewards[global_idx] = rewards
                        all_successes[global_idx] = success
                
                # Skip to next chunk (we've evaluated all envs in this chunk)
                continue
            ...
            while step_count < self.max_steps:
                ...
                # Check for success ONLY when episode ends (done or timeout)
                # This avoids counting intermediate is_success values
                if isinstance(done, (list, tuple, np.ndarray)):
                    for i in range(this_num_active_envs):
                        if i < len(done) and done[i]:
                            # Episode ended - now check final is_success
                            if isinstance(info, (list, tuple)) and i < len(info):
                                if isinstance(info[i], dict):
                                    is_success_val = info[i].get('is_success', 0)
                                    if isinstance(is_success_val, np.ndarray):
        ...
        # Logging - FIXED to use success rate
        max_rewards = collections.defaultdict(list)
        success_list = collections.defaultdict(list)  # NEW: Track success rates
        log_data = dict()
        ...
        for i in range(num_inits):
            ...
            # NEW: Get success flag
            is_success = 1.0 if all_successes[i] else 0.0
            
            max_rewards[prefix].append(total_reward)
            success_list[prefix].append(is_success)  # NEW
            log_data[prefix + f"sim_total_reward_{seed}"] = total_reward
            log_data[prefix + f"sim_success_{seed}"] = is_success  # NEW
