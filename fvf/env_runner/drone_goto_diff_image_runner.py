...
class DroneImageDiffRunner(BaseRunner):
    ...
    def _get_obs_from_env(self, env, image_buffer, keypoints_buffer):
        ...
        Returns:
            obs_dict: {"image": (1, T, C, H, W), "keypoints": (1, T, 9)}
        """
        ...
        keypointss = keypointss.reshape(keypointss.shape[0], -1)  # (T, 9) <-- 加这行
        
        # Add batch dimension
        obs_dict = {
            "image": images[None, ...],      # (1, T, C, H, W)
            "keypoints": keypointss[None, ...], # (1, T, 9) <-- 现在是 9 维
        }
